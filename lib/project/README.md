...
## Setting Up
- Copy the file ```example.deploy.yaml``` to ```deploy.yaml```.
- If your program uses environment variables, copy ```.env.example``` to ```.env```.
- Run ```npm run init``` to set up the deploy folder with the Terraform configurations
